...
public class ViewAllBookedDataScene implements IErrorMessage {
	...
	PCBook selectedPcBooked = new PCBook();
	LocalDate setLocalDate = LocalDate.now();
	ArrayList<String> pcListId = new ArrayList<>();
	...
	private void initializeAssignToNewPCButton() {
		...
		assignButton.setOnAction(event -> {
			String newPCId = pcListDropDown.getValue();
			PCBookController pcBookController = new PCBookController();
			String result = pcBookController.assignUserToPC(selectedPcBooked, newPCId);
	...
	private void addEventListener() {
		pcBookTV.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<PCBook>() {
			...
			public void changed(ObservableValue<? extends PCBook> observable, PCBook oldValue, PCBook newValue) {
				if(newValue != null) {
					selectedPcBooked.setBookID(newValue.getBookID());
					selectedPcBooked.setPcID(newValue.getPcID());
					selectedPcBooked.setUserID(newValue.getUserID());
					selectedPcBooked.setBookedDate(newValue.getBookedDate());
				}
			}
		});
	}
	...
	public void _repaint() {
		...
		PCBookController pcBookController = new PCBookController();
		pcBookedList = pcBookController.getAllPCBookedDataByDate(setLocalDate);
	    ...
	    for (PC pc : pcList) {
	        ...
	        for (PCBook pcBooked : pcBookedList) {
	            if (pc.getPcID().equals(pcBooked.getPcID())) {
	                isBooked = true;
	                break;
	            }
	        }
	    ...
	    pcListDropDown.setItems(FXCollections.observableArrayList(pcListId));
	}

...
public class ViewAllTransactionHistory implements IErrorMessage {
	...
	TableView<TransactionHistory> transactionsHistory;
	...
	private ViewAllTransactionHistory() {
		...
		container.getChildren().add(transactionsHistory);
	...
	private void initializeTransactionHistoryTable() {
		transactionsHistory = new TableView<TransactionHistory>();
		
		TableColumn<TransactionHistory, Integer> headerIdColumn = new TableColumn<>("Header ID");
        headerIdColumn.setCellValueFactory(new PropertyValueFactory<>("transactionHeader.transactionID"));
        
        TableColumn<TransactionHistory, Integer> staffIdColumn = new TableColumn<>("Staff ID");
        staffIdColumn.setCellValueFactory(new PropertyValueFactory<>("transactionHeader.staffID"));

        TableColumn<TransactionHistory, String> detailIdColumn = new TableColumn<>("Detail ID");
        detailIdColumn.setCellValueFactory(new PropertyValueFactory<>("transactionDetail.transactionID"));
        
        TableColumn<TransactionHistory, String> pcIdColumn = new TableColumn<>("PC ID");
        pcIdColumn.setCellValueFactory(new PropertyValueFactory<>("transactionDetail.pcID"));
        
        transactionsHistory.getColumns().add(headerIdColumn);
        transactionsHistory.getColumns().add(staffIdColumn);
        transactionsHistory.getColumns().add(detailIdColumn);
        transactionsHistory.getColumns().add(pcIdColumn);
        transactionsHistory.setPlaceholder(new Label("No Data Transactions History Records!"));
	}

	public void _repaint() {
		transactionsHistory.getItems().clear();
		TransactionController controller = new TransactionController();
		UserController userController = new UserController();
		User user = userController.getCurrentUser();

		List<TransactionHistory> result = controller.getUserTransactionDetail(user.getUserID());

		if (result != null) {
			for (TransactionHistory h: result) {
				transactionsHistory.getItems().addAll(result);
			}
		}
